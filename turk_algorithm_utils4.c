...
static void calculate_a_costs(t_stack *a, int index, int *cost_up, int *cost_down)
{
	*cost_up = a->top - index;
	*cost_down = index + 1;
}

static void calculate_b_costs(t_stack *b, int target_index, int *cost_up, int *cost_down)
{
	if (b->top == -1)
	{
		*cost_up = 0;
...
static t_move find_cheapest_tmove(int cost_a_up, int cost_a_down, int cost_b_up, int cost_b_down)
{
	t_move	moves[4];
	t_move	cheapest;
	int		i;
